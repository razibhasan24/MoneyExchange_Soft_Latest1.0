    ...
    <div class="card bg-info mb-3 p-4">
        <div class="row">
            <div class="col-12 d-flex justify-content-between align-item-center ">
                <h3 class=" card-title text-white d-flex align-items-center  m-0">Customer List</h3>
                <a href="{{ route('customers.create') }}" class="btn btn-light btn-sm" title="Create New Product">
                    <i class="fa fa-plus mr-1"></i> Create New Customer
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="card mb-3">
        ...
        <div class="table-responsive rounded-3">
            <table class="table table-hover">
                <thead class="table-primary"><tr><th>Id</th><th>Name</th><th>Id type</th><th>Id number</th><th>Phone</th><th>Address</th><th>Id proof document</th><th>Created at</th><th>Actions</th></tr></thead>
                <tbody>
                @foreach ($customers as $item)
                    <tr><td>{{ $item->id }}</td><td>{{ $item->name }}</td><td>{{ $item->id_type }}</td><td>{{ $item->id_number }}</td><td>{{ $item->phone }}</td><td>{{ $item->address }}</td><td>{{ $item->id_proof_document }}</td><td>{{ $item->created_at }}</td><td style="min-width:220px">
