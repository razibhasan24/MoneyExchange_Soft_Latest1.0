    ...
    <div class="card mb-3">
        ...
        <div class="table-responsive rounded-3">
            <table class="table table-hover">
                <thead class="table-primary"><tr><th>Id</th><th>Customer id</th><th>Invoice date</th><th>Total amount</th><th>Status</th><th>Actions</th></tr></thead>
                <tbody>
                @foreach ($invoices as $item)
                    <tr><td>{{ $item->id }}</td><td>{{ optional($item->customer)->name ?? $item->customer_id }}</td><td>{{ $item->invoice_date }}</td><td>{{ $item->total_amount }}</td><td>{{ $item->status }}</td><td style="min-width:220px">
