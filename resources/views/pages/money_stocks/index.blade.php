    ...
    <div class="card mb-3">
        ...
        <div class="table-responsive rounded-3">
            <table class="table table-hover">
                <thead class="table-primary"><tr><th>Id</th><th>Currency id</th><th>Qty</th><th>Transaction type</th><th>Remarks</th><th>Purchase id</th><th>Actions</th></tr></thead>
                <tbody>
                @foreach ($money_stocks as $item)
                    <tr><td>{{ $item->id }}</td><td>{{ optional($item->currency)->name ?? $item->currency_id }}</td><td>{{ $item->qty }}</td><td>{{ $item->transaction_type }}</td><td>{{ $item->remarks }}</td><td>{{ optional($item->purchase)->name ?? $item->purchase_id }}</td><td style="min-width:220px">
